...
export async function getStableHelmVersion(): Promise<string> {
  try {
    ...
    responseArray.forEach((response: any) => {
      if (response && response.tag_name) {
        ...
        if (currentHelmVerison) {
          if (
            currentHelmVerison.toString().indexOf('rc') == -1 &&
    ...
    core.warning(
      util.format(
        'Cannot get the latest Helm info from %s. Error %s. Using default Helm version %s.',
        helmAllReleasesUrl,
        error,
        stableHelmVersion
      )
...
export var walkSync = function (dir: any, filelist: any, fileToFind: any) {
...
async function getLatestHelmVersionFor(type: string): Promise<string> {
  ...
  try {
    ...
    let latestValidRelease = releases.find((release: any) =>
  ...
  core.warning(
    util.format(
      'Could not find stable release for Helm %s. Using default Helm version %s.',
      type,
      getStableHelmVersionFor(type)
    )
  );
...
function isValidVersion(version: string, type: string): boolean {
  if (!version.toLocaleLowerCase().startsWith(type)) return false;
